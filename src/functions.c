...
/** \fn void main_algorithm(int b[SIZE][SIZE], int a[SIZE][SIZE], int number_of_neighbors, int size_of_matrix,int *game_mode)
...
void main_algorithm(int b[SIZE][SIZE], int a[SIZE][SIZE], int number_of_neighbors, int size_of_matrix, int *game_mode) {
    ...
    for (y = 1; y < size_of_matrix; y++) {
        ...
        {
            if (b[x][y] == a[x][y]) count_equal++;


        }
    }
    if (count_equal > size_of_matrix*size_of_matrix - 2 * size_of_matrix) { count_equal = 0; (*game_mode) = 0; }
